...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        ...
        self.ema_modules = {}
        self.p_grad_data = {}
        self.dema = config["default"].dema
        self.base_weight = config["default"].base_weight
            ...
            if p.requires_grad and self.dema:
                ...
                with torch.no_grad():
                    try:
                        ...
                        alpha = float((1 + b) / (a * c) * 0.01)
                        if 0 < alpha < 1:
                            alpha = alpha
                        else:
                            alpha = self.base_weight
                        self.ema_modules[n].data = self.ema_modules[n].data * (1 - alpha) + p.data * alpha
                    except:
                        self.ema_modules[n].data = self.ema_modules[n].data * (1 - self.base_weight) + p.data * self.base_weight
