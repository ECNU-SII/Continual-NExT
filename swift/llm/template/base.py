...
class Template(ProcessorMixin):
    ...
    def _data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        # # 提取 index 字段
        # indices = [b.get('index') for b in batch if 'index' in b]
        # if indices:
        #     res['index'] = indices  # 保存并返回原始 index 列表
        ...
        extra_fields = {}
        # 提取所有已处理的字段名
        handled_keys = set(res.keys())
        # 找出所有未被处理的字段
        for feature in batch[0]:
            if feature not in handled_keys and feature not in keys:
                values = [b[feature] for b in batch]
                if 'input_ids' in res.keys():
                    extra_fields[feature] = torch.tensor(values).to(device=res['input_ids'].device)
                else:
                    extra_fields[feature] = torch.tensor(values).to(device=res['inputs_embeds'].device)
