...
class TrainArguments(SwanlabArguments, TunerArguments, BaseArguments, Seq2SeqTrainingOverrideArguments):
    ...
    EWC_limit: int = field(default=1000)
    EWC_path: str = field(default=None)
    
    # GEM
    GEM: bool = field(default=False) # 是否启用GEM方法。默认值：False
    GEM_memory_strength: float = field(default=0.5) # GEM投影约束的强度。较高的值会强制执行更强的约束。默认值：0.5
    GEM_replay_ratio: float = field(default=0.2) # 从每个过去任务中保留的样本比例。默认值：0.1
    GEM_replay_task_list: str = field(default=None) # 用作记忆的任务名称列表，以逗号分隔。默认值：None
    GEM_maxsamples_list: str = field(default=None) # 每个记忆任务要保留的最大样本数列表，以逗号分隔。默认值：None
    GEM_previous_task_dataset: str = field(default=None) # 包含先前任务数据集的目录路径。默认值：None
